...
def getdataframe():
    return pd.DataFrame(pd.read_csv(DatasetsDir + 'Ionosphere/ionosphere.data', header=None))
...
def load(**kwargs):
    ...
    class_names = dataframe.iloc[:, -1].unique()

    idx = 0
    print('class information:')
    for name in class_names:
        dataframe.iloc[:, -1].replace(name, idx, inplace=True)
